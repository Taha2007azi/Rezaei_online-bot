...
def main_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("مشاهده رزرو", callback_data="menu:view"), InlineKeyboardButton("ویرایش اطلاعات", callback_data="menu:edit")],
        [InlineKeyboardButton("انصراف (حذف رزرو)", callback_data="menu:cancel")],
    ])

# ====================== هندلرها (همه چیز توی پنجره) ======================
    ...
    if data == "menu:new":
        ...
        if _cursor:
            ...
            if rows and rows[0]['name']:
                user_info = rows[0]
        text = f"نام فعلی: {user_info['name']}\n\nنام جدید بفرستید یا همین رو دوباره بزنید." if user_info else "لطفاً نام و نام خانوادگی خود را وارد کنید:"
...
async def time_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _cursor:
        await db_execute("DELETE FROM appointments WHERE chat_id=%s", (chat_id,))
        await db_execute("""
            INSERT INTO appointments (chat_id, name, phone, age, issue, date, jalali_date, weekday, time, link, psych)
